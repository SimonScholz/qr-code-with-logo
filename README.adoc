...
The latest config is stored in a `config.json` file in your `user.home` folder inside `user.home/.qr-code-app` directory,
when closing the application.
On starup of the application this config is loaded and used to create a QR code.
...
The https://github.com/SimonScholz/qr-code-with-logo/releases[releases section] offers several different distributions for the `qr-code-app` project.

* Distribution, which constists of a `qr-code-app` or `qr-code-app.bat` file in the `/bin` folder, which can be executed directly. (requires Java to be installed on your system)
* JPackage distribution, which constists of a `bin/qr-code-app` or `qr-code-app.exe` file, which can be executed directly. (does *not* require Java to be installed on your system)
